...
\section{Approximating Top-$k$ Frequent Itemsets}
...
\item $\Smp \gets \emptyset$.
\item If $n\geq |\D|$, $\Smp \gets \D$; otherwise, choose $\Delta n$ itemsets from $\D$ at uniformly random. Denote by $\Delta \Smp$ the chosen $\Delta n$ itemsets.
...
\item If $\epsilon < d$, where $d$ is the distance between the frequencies of the $k$-th and $(k+1)$-th most frequent itemset in $\Ob$, or $\Smp = \D$, then return the top-$k$ results (tie-breaking when necessary). Otherwise, go to Step 3.
\end{enumerate}
\end{minipage}
}
