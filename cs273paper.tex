...
\subsection{Setup}
We implemented our algorithms by Python 3.4.3 and ran the programs on CSIL, a Fedora 20 (Linux) cluster at UCSB. To reduce the entire running time in the experiments (it is very time consuming to compute the exact solutions), for each dataset, we select the first 70 items and then approximate the frequencies of their pairs. By default we choose that the sample size increases by 100 for each round, and $\epsilon = 0.05$, $\delta = 0.01\%$.
...
\subsection{Results on Approximation with Precomputed Sample Size}
...
\hline
accidents & 20.74 & 3527 & 98\% & 364.66 & 63939 & 99\% \\
chess & 22.51 & 3196 & 100\% & 23.66 & 3196 & 100\% \\
connect &  &  &  &  &  &  \\
kosarak &  &  &  &  &  &  \\
mushroom & 19.51 & 3539 & 98\% & 115.66 & 22519 & 100\% \\
pumsb & 29.80 & 3539 & 95\% & 2160.51 & 224286 & 100\% \\
pumsb star &  &  &  &  &  &  \\
retail &  &  &  &  &  &  \\
\specialrule{1pt}{1pt}{1pt}
\end{tabular}
...
\subsection{Comparisons on Approximation with Progressive Sampling}
